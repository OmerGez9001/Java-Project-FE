...
public class WorkersPage extends JFrame {
    ...
    private JTable workersTable = null;
    ...
    public WorkersPage() {
        ...
        String[] columns = {"WorkerId", "Id", "Password", "PhoneNumber", "AccountNumber", "Job", "ShopId"};

        String[][] data = workers.stream().map(x -> new String[]{x.getWorkerId(), x.getId(), x.getPassword(), x.getPhoneNumber(), x.getAccountNumber(), String.valueOf(x.getJob()), String.valueOf(x.getShop().getId())}).toArray(String[][]::new);
        ...
        workersTable.getModel().addTableModelListener(e ->
        {
            if (e.getType() == TableModelEvent.UPDATE)
                try {
                    backendClient.upsertWorker(transformer.fromWorkersPage(workersTable, e.getFirstRow()));
